...
namespace ApiProject.WebUI.Models
{
    public class ChatHub : Hub
    {
        ...
        public async Task SendMessage(string userMessage)
        {
            ...
            await StreamChatLLM(history, Context.ConnectionAborted);
        }

        public async Task StreamChatLLM(List<Dictionary<string, string>> history, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(apiKey))
                throw new InvalidOperationException("RouteLLM apiKey is empty. Set apiKey constant in ChatHub.");

            // Not: Bu client'ı DI'da RouteLLM base URL ile tanımla:
            // BaseAddress = https://routellm.abacus.ai/v1/
            var client = _httpClientFactory.CreateClient("routellm");
            ...
            // RouteLLM OpenAI-uyumlu: /v1/chat/completions
